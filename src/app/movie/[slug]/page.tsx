...
import { MovieUnavailable } from '@/components/movie-unavailable';
import { RetryReset } from '@/components/retry-reset';
...
export default async function MoviePage({ params }: PageProps) {
  ...
  try {
    ...
    // TMDB unreachable → show the auto-retrying unavailable page, not a 404.
    if (error instanceof TMDBUnreachableError) {
      return <MovieUnavailable title={parsed.title} year={parsed.year} slug={slug} />;
    }
  ...
  if (!movie) return <MovieUnavailable title={parsed.title} year={parsed.year} slug={slug} />;
